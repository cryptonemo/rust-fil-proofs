...
use crate::stacked::{
    ...
    hash::hash2,
    params::{
        get_node, Encodings, EncodingsCache, PersistentAux, Proof, PublicInputs, ReplicaColumnProof,
...
impl<'a, H: 'static + Hasher, G: 'static + Hasher> StackedDrg<'a, H, G> {
    ...
    fn generate_layers(
        ...
        // 1 less, to account for the thread we are on, which is doing the encoding.
    ...
    pub(crate) fn transform_and_replicate_layers(
        ...
        let (store_encodings, column_hashes, tree_d) = crossbeam::thread::scope(|s| {
            // Generate key layers.
            let h = s.spawn(|_| Self::generate_layers(graph, layer_challenges, replica_id, true, config.clone()));
            ...
            let (store_encodings, column_hashes) = h.join().unwrap().unwrap();
            let column_hashes = column_hashes.unwrap();

            (store_encodings, column_hashes, tree_d)
        })?;

        // Convert store_encodings from a Vec<StoreConfig> to a Vec<DiskStore<H::Domain>>
        let encodings: EncodingsCache<H> = EncodingsCache::new(&store_encodings);
